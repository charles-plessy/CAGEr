...
#		return("'genomeName' must be a name of one of the genome packages available in BSgenome! See 'available.genomes()'")
		...
		if(!(object@inputFilesType %in% c("bam", "ctss", "FANTOM")))
		return("'inputFilesType' must be one of supported input file types (\"bam\", \"ctss\")!")
		if(length(object@sampleLabels) != length(object@inputFiles)) {
			return("Number of provided sample labels must match the number of input files!")
		}
