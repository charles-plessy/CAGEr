...
#' 
#' @description Creates BED file(s) with track(s) of individual CTSSs, tag clusters or
#' consensus clusters.  CTSSs and consensus clusters can be optionally colored in the color of
#' their expression class.  Tag clusters and consensus clusters can be displayed in a gene-like
#' representation with a line showing full span on the cluster, filled block showing
#' interquantile range and a thick box denoting position of the dominant (most frequently used)
#' TSS.
#' 
#' @param object A \code{\link{CAGEr}} object.
#' 
#' @param what Which elements should be exported to BED track.  Can be \code{"CTSS"} to export
...
#' Default value \code{NULL} uses start (resp. end) position of the cluster.  Ignored when
#' \code{what = "CTSS"}.  See Details.
#' 
#' @param colorByExpressionProfile Logical, should blocks be colored in the color of their
#' corresponding expression class. Ignored when \code{what = "tagClusters"}.
#' 
#' @param oneFile Logical, should all CAGE datasets be exported as individual tracks into the
#' same BED file (TRUE) or into separate BED files (FALSE).  Ignored when \code{what = "CTSS"},
#' which by default produces only one track.
#' 
#' @details This function creates various representations of CTSSs, tag clusters and consensus
#' clusters in the BED format, which can be directly visualised in the UCSC Genome Browser.
#' 
#' When \code{what = "CTSS"}, one BED file with single track of 1 bp blocks representing all
...
#' \code{\link{getExpressionProfiles}} function).  Colors of expression classes match the colors
#' in which they are shown in the plot returned by the \code{\link{plotExpressionProfiles}}
#' function.  For \code{colorByExpressionProfile = FALSE}, CTSSs included in the clusters are
#' shown in black and CTSSs that were filtered out in gray.
#' 
#' When \code{what = "tagClusters"}, one track per CAGE dataset is created, which can be
#' exported to a single BED file (by setting \code{oneFile = TRUE}) or separate BED files
#' (by setting \code{oneFile = FALSE}).  In case \code{qLow = NULL} and \code{qUp = NULL},
#' TCs are represented as simple blocks showing only the full span of TC from  the start to
#' the end.  Setting \code{qLow} and/or \code{qUp} parameters to a value of the desired quantile
...
#' color.
#' 
#' When \code{what = "consensusClusters"} consensus clusters are exported to BED file.  Since
#' there is only one set of consensus clusters common to all CAGE datasets, only one track is
#' created in case of a simple representation.  This means that when \code{qLow = NULL} and
#' \code{qUp = NULL} one track with blocks showing the full span of consensus cluster from
...
#' positions of quantiles and dominant TSS.  Thus, when \code{qLow} and/or \code{qUp} parameters
...
#' setting \code{oneFile = TRUE}) or separate BED files (by setting \code{oneFile = FALSE}).
...
#' \code{\link{getExpressionProfiles}} function).  Colors of expression classes match the colors
#' in which they are shown in the plot returned by the \code{\link{plotExpressionProfiles}}
#' function.  For \code{colorByExpressionProfile = FALSE} all consensus clusters are shown in
#' black.
#' 
#' @return Creates BED file(s) in the working directory that can be directly visualised as
