...
\begin{itemize}
\item
Reading in multiple CAGE datasets from various sources; user provided BAM or TSS input files, public CAGE datasets from accompanying data packages.
\item
Normalizing raw CAGE tag count: simple tag per million (tpm) or power-law based normalization (\cite{Balwierz:2009}).
\item
Clustering of individual TSSs into tag clusters (TCs) and aggregating clusters across multiple CAGE datasets to create a set of consensus promoters.
\item
...
\item
Scoring differential TSS usage (promoter shifting) and detecting promoters that shift between two samples.
\end{itemize}

Two data packages are accompanying this package: \Rpackage{FANTOM3and4CAGE} and \Rpackage{ENCODEprojectCAGE}. 
They contain majority of the up-to-date publicly available CAGE data produced by two major genome regulation-oriented international 
consortia, FANTOM and ENCODE, respectively, and provide a valuable resource of genome-wide TSSs in various organisms, 
tissues and cell types that can be used directly in R. Section 5 in this vignette describes how these public datasets can be included into a
workflow provided by \Rpackage{CAGEr}. See the vignettes of these data packages for the list of available CAGE datasets.
...
\end{itemize}
\item
\textbf{Publicly available CAGE datasets from R data packages}: Two data packages containing CAGE data produced by FANTOM and ENCODE 
consortia (\Rpackage{FANTOM3and4CAGE} and \Rpackage{ENCODEprojectCAGE}, respectively) are accompanying this 
package. Selected subset of these data can be used as input for \Rpackage{CAGEr}.
\end{enumerate}
...
order in all the results and plots.
After the data has been read we might want to merge and/or rearrange some of the datasets. To rearrange the samples in the
temporal order of the zebrafish development (unfertilized egg -> high -> 30 percent dome -> prim6) and to merge the two replicates for the 
...
@
This will create a PDF file with reverse cumulative plots (Figure~\ref{fig:ReverseCumulatives}) in your working directory. The plots can help in choosing 
the parameters for power-law based normalization. We can see that the reverse cumulative distributions look similar and follow the power-law in the central part of the CAGE 
tag counts values with a slope between -1.2 and -1.3. Thus, we choose a range from 5 to 500 tags to fit a power-law, and we normalize all samples to a referent power-law distribution with a 
total of 50,000 tags and slope of -1.25 (\Rcode{alpha = 1.25}). (Note that since this example dataset contains only data from one part of chromosome 17 and the total number of tags is very small, 
we normalize to a referent distribution with a similarly small number of tags. When analyzing full datasets it is reasonable to set total number of tags for referent distribution to one million to get 
normalized tags per million values.)
...
normalizeTagCount(myCAGEset, method = "powerLaw", 
		fitInRange = c(5, 500), alpha = 1.25, T = 5*10^4)
@
...
Tag clusters and their interquantile width can be visualized in a gene-like representation in the UCSC genome browser by exporting the data into BED file:
<<eval=FALSE>>=
...
globally compare the promoter width between different samples (Figure~\ref{fig:TagClustersInterquantileWidth}):
<<eval=FALSE>>=
...
shifting score is calculated based on the difference in the cumulative distribution of CAGE signal along that promoter in the two samples. Thus, prior to shifting score calculation, we have to 
...
@
Next, we calculate shifting score for all promoters comparing two specified samples:
<<>>=
...
We can select a subset of promoters with shifting score above specified threshold:
<<>>=
shifting.promoters <- getShiftingPromoters(myCAGEset, 
		tpmThreshold = 5, scoreThreshold = 0.6)
head(shifting.promoters)
@
The \Rfunction{getShiftingPromoters} function returns genomic coordinates and shifting score of the promoters, as well as the value of CAGE signal and position of the dominant TSS in the two 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Two data packages containing CAGE data produced by FANTOM and ENCODE consortia are accompanying this package. 
These are \Rpackage{FANTOM3and4CAGE} and \Rpackage{ENCODEprojectCAGE}, respectively. For the full list and description of the 
available datasets within these packages and references to the original publications of the containing data, please refer to the vignettes of 
the corresponding packages. In this vignette we will only demonstrate how to include a user-selected subset of the provided datasets into 
the \Rpackage{CAGEr} workflow.
We will use CAGE data produced by FANTOM consortium for various human tissues. To load the corresponding data package type:
<<>>=
