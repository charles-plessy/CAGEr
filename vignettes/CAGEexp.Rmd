...
can also display natively data from BAMor BED files as coverage tracks.].

```{r}
# FIXME: export GrangesList of CTSSes, split them by strand, put strand name
# set negative scores on negative strand,
# in track name, and show how to loop on the object or provide a function for that
# FIXME make CTSSnormalizedTpmGR it able to loop over samples, or create CTSSnormalizedTpmGRL
trk <- exportToTrack(CTSSnormalizedTpmGR(ce, "Zf.30p.dome"))
# export(trk[strand(trk) == "+"], con = "fixme", format = "ucsc", subformat = "bedGraph", append = TRUE)
# Fixme: restore the function to do the aboce.
...
```{r}
### FIXME.... toooo slooooooowwwww !!!
exportToTrack(ce, what = "tagClusters", qLow = 0.1, qUp = 0.9, oneTrack = TRUE)
```
...
FIXME! Each cluster is shown in different color and is marked by its label and the
