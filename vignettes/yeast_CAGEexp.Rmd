...
# this is for now local paths, until we see how we wiill host the data
inputFiles = list.files(path = "~/data/yeast_cage/my_map/", 
...
```{r create_CAGEexp}
ce <- CAGEexp( genomeName     = "BSgenome.Scerevisiae.UCSC.sacCer3"
...
```{r load_data}
ce <- getCTSS(ce, correctSystematicG = F, removeFirstG = T)
ce
```
...
```{r}
ce <- mergeSamples(ce, mergeIndex = c(1,1,2,2,3,3,4,4), 
                   mergedSampleLabels = c("an", "et", "gl", "nl"))
ce <- annotateCTSS(ce, exampleZv9_annot)
```
...
```{r}
ce <- normalizeTagCount(ce, method = "powerLaw", fitInRange = c(5, 2000), 
                        alpha = 1.05, T = 1e+06)
ce[["tagCountMatrix"]]
```
...
```{r}
plotInterquantileWidth(ce, clusters = "tagClusters", 
...
```{r}
ce <- aggregateTagClusters(ce, tpmThreshold = 5, 
...
```{r}
consensusClustersGR( ce, sample = "an"
		               , returnInterquantileWidth = TRUE,  qLow = 0.1, qUp = 0.9)
```
